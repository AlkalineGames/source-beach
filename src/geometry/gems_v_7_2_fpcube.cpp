	...
	if(already_know_verts_are_outside_cube) {
		...
		for(i=0; i<nverts; i++) {
			...
			if(0L == face_bits)  /* vertex is inside the cube */
            {
                ...
                if ( is_cylinder_test )
                {
                    Real yRadiusSquared = (verts[i][0] * verts[i][0] * 4)
                                        + (verts[i][2] * verts[i][2] * 4);
                    if ( yRadiusSquared <= (Real)0.9 ) // 1.0 - ~0.05 to allow for rounding errors
                        return 1; // trivial accept: vertex is inside unit cylinder
                }
                else
            ...
            int is_cylinder_test) // ### added by World Fusion
{
	int quick_test = trivial_vertex_tests(nverts, verts,
				already_know_verts_are_outside_cube, is_cylinder_test);
	if(-1 == quick_test)
		return polygon_intersects_cube(nverts, verts, polynormal, 1,
				already_know_edges_are_outside_cube, is_cylinder_test);
	else
		return quick_test;
}
